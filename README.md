...
| 模块 | 说明 |
|------|------|
| 多目录文件搜索 | 管理多个工作目录（带“搜索目录/关键字/扩展名/排除文件夹”标签）；支持普通字符串、通配符（`* ?`）、正则；按扩展名筛选；排除文件夹/文件名；结果含文件名、路径、修改时间、大小，可双击打开 |
| 文件夹快链 | 添加/删除/重命名/排序（上移/下移）快链；在资源管理器打开；与工作目录独立存储，支持导出/导入 |
...
| 凭证保管 | 条目管理（名称/账号/密钥/备注），支持新增/编辑/删除；密码默认掩码；一键复制到剪贴板；敏感字段使用 Windows DPAPI 加密存储 |
